...
#define DELAY_MS 60000 // Delay in miliseconds before reading another telegram (WAS 60000)
#define WAIT_FOR_DATA_MS 2000 // WAS 2000
...
class CustomP1UartComponent : public Component, public uart::UARTDevice {
 protected:
  ...
  bool read_message() {
	...
	ESP_LOGD("ROWDY", "Start reading message");
	if (available()) { // Check to be sure
		ESP_LOGD("ROWDY", "Available");
					...
					if (c == 33) { // footer: exclamation mark
						...
						if (footerfound && c == 10) { // last \n after footer
							...
							if (res.err) {
								...
								ESP_LOGE("ROWDY", "Error reading telegram!");
								Serial.println(res.fullError(telegram, telegram + telegramlen));
							} else {
								ESP_LOGD("ROWDY", "Publish data");
  ...
  void loop() override {
	...
	if (now - lastread > DELAY_MS || lastread == 0) {
		...
		if (data_available()) { // Check for x seconds if there's data available
		    ESP_LOGD("ROWDY", "Data available");
			bool have_message = read_message();
			if (have_message) { 
                ESP_LOGD("ROWDY", "Message found");			
				...
				ESP_LOGD("ROWDY", "No message found. Keep output port high");
			}
			
		} else {
				ESP_LOGD("DMSR","No data available. Is P1 port connected?");
		}	
	}
  }	
